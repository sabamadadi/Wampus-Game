...
class WumpusWorldEnv(gym.Env):
    def __init__(self, size=4, number_of_pits=3, max_steps=100):
        ...
        while self.wumpus_pos == self.gold_pos or self.wumpus_pos == self.pit_pos[0] or self.wumpus_pos == self.pit_pos[1] or self.wumpus_pos == self.pit_pos[2] or self.wumpus_pos == self.pit_pos[0] or self.gold_pos == self.pit_pos[0] or self.gold_pos == self.pit_pos[1] or self.gold_pos == self.pit_pos[2] or self.wumpus_pos == self.agent_pos or self.agent_pos == self.pit_pos[0] or self.agent_pos == self.pit_pos[1] or self.agent_pos == self.pit_pos[2] or self.gold_pos == self.agent_pos:
            self.wumpus_pos = (np.random.randint(self.size), np.random.randint(self.size))
            self.gold_pos = (np.random.randint(self.size), np.random.randint(self.size))
            self.pit_pos = [(np.random.randint(self.size), np.random.randint(self.size)) for _ in range(self.number_of_pits)]
        ...
        self.temp_wamp_pos = self.wumpus_pos
        self.done = False
        self.hit_wampus_reward = 10
    ...
    def step(self, action):
        ...
        print(self.wampus_pos)

        a = self._update_agent_position(action)
        reward = 0
        if a:
            reward = self.wrong_move
        else:
    ...
    def _calculate_reward(self):
        ...
        if self.agent_pos == self.wumpus_pos or self.agent_pos in self.pit_pos:
            self.done = True
            return -20  # Penalty for falling into pit or encountering Wumpus
        elif self.agent_pos == self.gold_pos:
            self.done = True
            return 30  # Reward for grabbing gold
        else:
    ...
    def reset(self):
        ...
        self.wampus_pos = self.temp_wamp_pos
        print(self.wampus_pos)
